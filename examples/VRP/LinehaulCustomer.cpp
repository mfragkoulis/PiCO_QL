...
// Returns true if all Linehaul customers are serviced, flase otherwise.
...
// Erases a linehaul customer from the active list when picked for service.
...
void LinehaulCustomer::erase_c(int pos, int i) {
    ...
    /* cout << "1-Linehaul customer : " << (*list_a)[pos]->get_code() << 
       " is being erased from position " << pos << " of " << 
       list_a->size() << " element list" << endl; */
    (*list_a)[pos]->set_serviced();
    list_b->push_back((*list_a)[pos]);
    list_a->erase(list_a->begin() + pos);
    /* cout << "Now list " << i%2 << " contains : " << list_a->size() << 
       " elements." << endl; */
}

// Returns the distance between a pair of linehaul customers.
double LinehaulCustomer::get_dist(string pair) {
    return dist_l[pair];
}
 
// Computes the distance between all pairs of linehaul customers.
void LinehaulCustomer::compute_dist() {
	...
	for (nested=next; nested!=coord_l.end(); nested++) {
	    double temp = sqrt( pow( nested->second->get_x() - 
				     top->second->get_x(),2 ) + 
				pow( nested->second->get_y() - 
				     top->second->get_y(),2 ) );
	    dist_l.insert(make_pair(top->first + nested->first, temp));
	    /* cout << "The distance between " << top->first << 
	    " and " << nested->first << 
	    " linehaul participators is : " << 
	    LinehaulCustomer::get_dist(top->first + nested->first) << 
	    " units" <<  endl; */
	}
    }
}
