void cTrackDB_cpp_query2() {
  ...
  std::map<int, std::multimap<float, std::pair<float, std::pair<QString, CTrack*>* > > > interim;
  // maxAvgspeed, elevation, heartRateBpm                   
  std::multimap<float, std::pair<float,std::pair<int,std::pair<QString, CTrack*>* > > > aggregate;
  ...
  std::multimap<float, std::pair<float, std::pair<QString, std::pair<float, std::pair<float, std::pair<QString, CTrack*>* > > pairEle;
  std::pair<float, std::pair<QString, CTrack*>* > pairAvg;
  bool inserted = false;
  std::pair<int, std::pair<QString, CTrack*>* > pairHr;
  std::map<int, std::multimap<float, std::pair<float, std::pair<QString, CTrack*>* > > >::iterator interimIt;
  std::multimap<float, std::pair<float, std::pair<QString, CTrack*>* > >::iterator mapEleIt, mapEleItNext;
  std::pair<QString, CTrack*>* trackMax = NULL;
  int mapEleSum = 0;
  int ele_equal = 0;
  ...
  for (int i = 0; i < 10; i++) {
    ...
    for (rsIt = resultset.begin(); rsIt != resultset.end();\
 rsIt++) {
      ...
      for (; groupByNext != (*rsIt).end(); groupByNext++) {
        ...
        if (currentHr == nextHr) {
          pairAvg = make_pair (currentAvgSp, (std::pair<QString, CTrack *>*)&*iter);
          mapEle.insert(std::pair<float, std::pair<float, std::pair<QString, CTrack*>* > >(currentEle, pairAvg));
          inserted = true;
          ...
          interim.insert(std::pair<int, std::multimap<float, std::pair<float, std::pair<QString, CTrack*>* > > >(currentHr, mapEle));
          mapEle.clear();
          //std::cout << "Inserted in INTERIM heartrate: " << currentHr << std::endl;                                  
        }
        groupBy++;
      }
      pairAvg = make_pair ((*groupBy).second.second, (std::pair<QString, CTrack *>*)&*iter);
      mapEle.insert(std::pair<float, std::pair<float, std::pair<QString, CTrack*>* > >(nextEle, pairAvg));
      //pairAvg = make_pair (nextEle, (std::pair<QString, CTrack *>*)&*iter);                                          
      interim.insert(std::pair<int, std::multimap<float, std::pair<float, std::pair<QString, CTrack*>* > > >(nextHr, mapEle));
    ...
    for (interimIt = interim.begin(); interimIt != interim.end(); interimIt++) {
      ...
      for (; mapEleItNext != (*interimIt).second.end(); mapEleItNext++) {
        ...
        if (currentEle == nextEle) {
          ...
          if (maxAvgspeed > 0) {
            pairHr = make_pair (currentHr, (std::pair<QString, CTrack *>*)trackMax);
            pairEle = make_pair (currentEle, pairHr);
            aggregate.insert(std::pair<float, std::pair<int, std::pair<float, std::pair<QString, CTrack*>* > > >(maxAvgspeed, pairEle));
      ...
      if (ele_equal == 1) {
        ...
        if (maxAvgspeed > 0) {
          pairHr = make_pair (currentHr, (std::pair<QString\
, CTrack *>*)trackMax);
          pairEle = make_pair (currentEle, pairHr);
          aggregate.insert(std::pair<float, std::pair<int, std::pair<float, std::pair<QString, CTrack*>* > > >(maxAvgspeed, pairEle));
    ...
    std::multimap<float, std::pair<float, std::pair<int, std::pair<QString, CTrack*>* > > >::reverse_iterator aggrRIt;
    CTrack *currentTrack;
    ...
    for (aggrRIt = aggregate.rbegin(); aggrRIt != aggregate.rend(); aggrRIt++) {
      ...
      currentTrack = (*aggrRIt).second.second.second->secon\
d;
